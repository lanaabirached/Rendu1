					...
					unsigned short* energy, unsigned short* nb_disques)
{
	...
	if (sscanf(line, "%"xstr(MAX_PSEUDO)"s%n %d %d %d", pseudo, &cursor,
				   ...
				   unsigned short* nDisk, v2d_t** position, unsigned short** value)
{
	...
	if (sscanf(line, "%f %f %d", &x, &y, &value[p][d]) != 3)
	{
	...
	for (i = 0; i < p; i++) {
		for (j = 0; j < nDisk[i]; j++) {
			...
			{
				error_disks_are_in_contact(0, j, 2, d);
				return false;
			}
		}
	}
	...
	for (i = 0; i < d; i++) {
		...
		if (sumRadius >= distance)
		{
			error_disks_are_in_contact(p+1, i, p+1, d);
			return false;
		}
	}
